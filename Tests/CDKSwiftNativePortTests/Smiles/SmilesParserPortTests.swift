...
final class SmilesParserPortTests: XCTestCase {
    ...
    func testParsesBracketDecoratorFieldsInStrictMode() throws {
        ...
        XCTAssertEqual(atom.explicitHydrogenCount, 3)
        XCTAssertEqual(atom.substitutionCount, 1)
        XCTAssertEqual(atom.unsaturated, 1)
        XCTAssertEqual(atom.ringBondCount, 0)
        XCTAssertEqual(atom.atomClass, 12)
        XCTAssertEqual(atom.atomMapNumber, 12)
    }
    ...
    func testRejectsUnsupportedBracketDecoratorInStrictMode() {
        XCTAssertThrowsError(try parse("[C$]"))
    }
